...
function getQuestionaryAnswerQuery($QuestionarieID, $QuestionID, $userID, $answer)
{
  ...
  $query .= " VALUES (NULL , '$QuestionarieID', '$QuestionID', '$userID', '$answer', ;placeholderß);";
  return $query;
}
...
function getAllRuinsWithExercises()
{
  ...
  $categoryArray = array();
  while ($row = mysql_fetch_assoc($result)) {
    /* minden id?zetet csak egyszer adunk hozz? az eredm?nyhez */
...
function getCurrentStory($story_ID)
{
  ...
  while ($row = mysql_fetch_assoc($result)) {
    ...
    /* minden sikert?rt?netet csak egyszer adunk hozz? az eredm?nyhez */
...
function getCategoryName($category)
{
  switch ($category) {
    case 1:
      $categoryName = '?NISMERET, ?NMEGVAL?S?T?S';
      break;
    case 2:
      $categoryName = 'P?RKAPCSOLAT';
      break;
    case 3:
      $categoryName = 'CSAL?D';
      break;
    case 4:
      $categoryName = 'MUNKA, P?NZ';
      break;
    case 5:
      $categoryName = 'KAPCSOLATOK, KOMMUNIK?CI?';
      break;
    case 6:
      $categoryName = '?ZLETI/VEZET?I K?PESS?GEK';
      break;
    default:
...
function sendSuccessFormMail($subChapterId, $emailGroup = 'all', $pictureName)
{
  ...
  for ($i = 0; $i < count($allUsers); $i++) {
    if ($allUsers[$i]['language'] == "1") {
      $hitext = "Szia";
      $text = "Azonnali bel?p?shez klikkelj ide!";
    } else {
      $hitext = "Dear";
  ...
  print "<script>";
  if ($success) {
    print "alert('" . $sentMails . " db ?zenet k?ld?se siker?lt!')";
  } else {
...
function storeNewUser($userArray, &$message)
{
  ...
  if ($number > 0) {
    $message = "Ez a jelsz? m?r foglalt, v?lassz m?sikat!";
    return false;
  }
...
function createUser($userArray, &$message)
{
  if (!$userArray['username'] or !$userArray['vezeteknev'] or !$userArray['keresztnev'] or !$userArray['email']) {
    $message = "A vezet?kn?v, keresztn?v, username ?s email kit?lt?se k?telez?!";
    return false;
  }
  foreach ($userArray as $key => $value) {
    $userArray[$key] = str_replace("'", "''", $value);
...
?>
  ...
  <table border='0'>
    <tr>
      <td valign='top' align='center' width='86' height='16'><img border="0" src=<?php print "'" . getImgSrcNameForMenuBalls('communityLife') . "'"; ?> style='cursor:pointer' onclick="menuWork('communityLife');"></td>
      <td valign='center' align='left' width='138' style="font-size:14px;">
        <font color='#574b21'><span style='cursor:pointer' onclick="menuWork('communityLife');"><?php print translate('Community_life'); ?></span>
      </td>
    </tr>
    <tr>
      <td valign='top' align='center' width='86' height='16'><img border="0" src=<?php print "'" . getImgSrcNameForMenuBalls('money') . "'"; ?> style='cursor:pointer' onclick="menuWork('money');"></td>
      <td valign='center' align='left' width='138' style="font-size:14px;">
        <font color='#87268b'><span style='cursor:pointer' onclick="menuWork('money');"><?php print translate('Money'); ?></span>
      </td>
    </tr>
    <tr>
      <td valign='top' align='center' width='86' height='16'><img border="0" src=<?php print "'" . getImgSrcNameForMenuBalls('soulTools') . "'"; ?> style='cursor:pointer' onclick="menuWork('soulTools');"></td>
      <td valign='center' align='left' width='138' style="font-size:14px;">
        <font color='#190d7f'><span style='cursor:pointer' onclick="menuWork('soulTools');"><?php print translate('Tools_for_the_soul'); ?></span>
      </td>
    </tr>
    <tr>
      <td valign='top' align='center' width='86' height='16'><img border="0" src=<?php print "'" . getImgSrcNameForMenuBalls('vitalProducts') . "'"; ?> style='cursor:pointer' onclick="menuWork('vitalProducts');"></td>
      <td valign='center' align='left' width='138' style="font-size:14px;">
        <font color='#008a98'><span style='cursor:pointer' onclick="menuWork('vitalProducts');"><?php print translate('Vital_Products'); ?></span>
      </td>
    </tr>
    <tr>
      <td valign='top' align='center' width='86' height='16'><img border="0" src=<?php print "'" . getImgSrcNameForMenuBalls('livingDiet') . "'"; ?> style='cursor:pointer' onclick="menuWork('livingDiet');"></td>
      <td valign='center' align='left' width='138' style="font-size:14px;">
        <font color='#289a51'><span style='cursor:pointer' onclick="menuWork('livingDiet');"><?php print translate('Living_the_diet_and_lifestyle'); ?></span>
      </td>
    </tr>
    <tr>
      <td valign='top' align='center' width='86' height='16'><img border="0" src=<?php print "'" . getImgSrcNameForMenuBalls('personalizedPh') . "'"; ?> style='cursor:pointer' onclick="menuWork('personalizedPh');"></td>
      <td valign='center' align='left' width='138' style="font-size:14px;">
        <font color='#999920'><span style='cursor:pointer' onclick="menuWork('personalizedPh');"><?php print translate('Your_personalised_pH_program'); ?></span>
      </td>
    </tr>
    <tr>
      <td valign='top' align='center' width='86' height='16'><img border="0" src=<?php print "'" . getImgSrcNameForMenuBalls('bloodAnalysis') . "'"; ?> style='cursor:pointer' onclick="menuWork('bloodAnalysis');"></td>
      <td valign='center' align='left' width='138' style="font-size:14px;">
        <font color='#8e5500'><span style='cursor:pointer' onclick="menuWork('bloodAnalysis');"><?php print translate('Blood_analysis'); ?></span>
      </td>
    </tr>
    <tr>
      <td valign='top' align='center' width='78' height='16'><img border="0" src=<?php print "'" . getImgSrcNameForMenuBalls('basicUnderstandings') . "'"; ?> style='cursor:pointer' onclick="menuWork('basicUnderstandings');"></td>
      <td valign='center' align='left' width='138' style="font-size:14px;">
        <font color='#7f1f1f'><span style='cursor:pointer' onclick="menuWork('basicUnderstandings');"><?php print translate('Understanding_the_basics'); ?></span>
      </td>
    </tr>
  ...
  ?>
      ...
      <tr>
        ...
        <td style="font-size:14px;" valign='top'><br>
          ...
          <a href='http://www.luciendelmar.com/communitylife.php'><b>
              <font color='#5cacc0'><?php echo translate('PTO') ?> ...</a>
        </td>
      </tr>
    <?php
      break;
    ...
    ?>
      ...
      <tr>
        <td align='right' valign='top'><a href='http://www.luciendelmar.com/livingthelifestyle.php'><b>
              <font color='#5cacc0'><?php echo translate('PTO') ?> ...</a></td>
      </tr>
    <?php
      break;
    case 'personalizedPh':
    ...
    ?>
      ...
      <tr>
        <td align='right' valign='top'><a href='http://www.luciendelmar.com/verelemzes.php'><b>
              <font color='#5cacc0'><?php echo translate('PTO') ?> ...</a></td>
      </tr>
    <?php
      break;
...
function storeOrder($fields)
{
  if (!$fields['varos'] or !$fields['vezeteknev'] or !$fields['keresztnev'] or !$fields['email'] or !$fields['cim']) {
    $message = "A vezet?kn?v, keresztn?v, v?ros, c?m ?s email kit?lt?se k?telez?!";
    return false;
  }
  foreach ($fields as $key => $value) {
...
if (!function_exists('getPostElement')) {
  function getPostElement($name)
  {
