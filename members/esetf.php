<?php
	...
	if($isTest) {	
		...
		//PROGRAMRÉSZ: HOGY TÖBB TANÁCSADÓ IS HASZNÁLHASSA AZ ÉLELMISZERLISTA GENERÁTORT
...
<head>
    ...
    <style>		
	div.note1, div.note2, div.note22, div.note3, div.addConsultation, div.specCons {
		float: right;
	...
	#divQuestFill, #divNote1 {
		width: calc(50vw - 15px);
		height:calc(100vh - 70px);
		overflow: auto;
	}
	
	#divNote2, #divNote22, #divNote3 {		
		width: 100%;
	}
	#divUserPlate{
		position: fixed;
	...
	.generateButtonOld, .foodListGeneration {
		text-align: right;
	}
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $("#btnDone").click(function () {
                $.post("sendAjanlas.php", { id: fillId, type: 1, acstipus: getRadioValue("acstipus") });
                if(fillId !== null)
			...
			$("body").on("click", "tr.answered", function () {
				...
				var obj = $(this);
				$.post("changeHighlight.php", { answerId: answerId, isHighlighted: !isHighlighted }, function (result) {
			...
			$("body").on("click", "div.addConsultation", function (e) {
				...
				if (confirm("Biztos fel szeretnél venni egy új konzultációt?") == true) {
					...
					$.post("addNewConsultation.php", { id: contDiv.data("myid") }, function (result) {
			...
			$("#btnSearch").click(function () {
				...
				if(exp.length == 0){					
					...
					$.get("getFilteredUsers.php", { txt: exp }, function (data) {						
			...
			$("#btnAddOrphanUser").click(function () {				
				...
				$.post("saveOrphanUser.php", { userName: userName }, function () {
					location.reload();										
				});
			});
			...
			$("#btnAddTestUser").click(function () {
				...
				if(uName.length == 0){					
					alert("Kérlek add meg az új ügyfél nevét.");
				}
				else{
					$.get("refreshTestUsers.php", { userName: uName, consId: uConsultantId }, function (data) {
						location.reload();
					});
				}
			});
			...
			$("#btnGenFoodRecom").click(function () {					
				...
				// ITT HOZZUK LÉTRE A VÁLTOZÓT A NYELVVÁLTÁSHOZ
				var lang = '';
			...
			$("#divRootCauses").on("change", ".cbRootCauses", function (){
				$.post("storeCheckbox.php", { type: 1, fillId: fillId, value: $(this).val(), isChecked: ($(this).prop("checked") ? 1 : 0) }, function (result) {
					notify("Mentve", 1000);
				});				
			});
			$("#divRootCauses").on("change", "#rootCauseOther", function (){
				$.post("storeText.php", { type: 1, fillId: fillId, value: $(this).val() }, function (result) {
					notify("Mentve", 1000);
				});				
			});			

			$("#divSolutionSteps").on("change", ".cbSolutionSteps", function (){
				$.post("storeCheckbox.php", { type: 2, fillId: fillId, value: $(this).val(), isChecked: ($(this).prop("checked") ? 1 : 0) }, function (result) {
					notify("Mentve", 1000);
				});				
			});

			$("#divEatMore").on("change", ".cbEatMore", function (){
				$.post("storeCheckbox.php", { type: 3, fillId: fillId, value: $(this).val(), isChecked: ($(this).prop("checked") ? 1 : 0) }, function (result) {
					notify("Mentve", 1000);
				});				
			});
			
			$("#cbIsGlutenmentes").click(function (){ checkFoodByCat(1, this); });
			$("#cbIsCsucsorfele").click(function (){ checkFoodByCat(2, this); });
			$("#cbIsOlajosMag").click(function (){ checkFoodByCat(3, this); });
			$("#cbIsMagasHisztamin").click(function (){ checkFoodByCat(4, this); });
			$("#cbIsMagasGI").click(function (){ checkFoodByCat(5, this); });
			$("#cbIsMagasFruktoz").click(function (){ checkFoodByCat(6, this); });
			$("#cbIsMagasFodmap").click(function (){ checkFoodByCat(7, this); });
			$("#cbIsKozepesFodmap").click(function (){ checkFoodByCat(8, this); });
			$("#cbIsTejKereszt").click(function (){ checkFoodByCat(9, this); });
			$("#cbIsHuvelyesek").click(function (){ checkFoodByCat(10, this); });
			$("#cbIsTejtermekek").click(function (){ checkFoodByCat(11, this); });
			$("#cbIsHusok").click(function (){ checkFoodByCat(12, this); });
			$("#cbIsTojas").click(function (){ checkFoodByCat(13, this); });
			$("#cbIsRefluxTr").click(function (){ checkFoodByCat(14, this); });
			$("#cbIsLektin").click(function (){ checkFoodByCat(15, this); });
		...
		function checkFoodByCat(type, obj){			
			$.get("getElelmList.php", { type: type }, function (data){
				if(data.length > 0){
					for(var i = 0; i < data.length; i++){
		...
		function selectFillRow(obj){
			...
			if(isTest){
				...
				newObj.css({ "background-color": "white" });
		...
		function storeNote(noteType) {			
			...
			$.post("storeNote.php", { noteType: noteType, note: val, fillId: fillId }, function (result) {
				notify("Mentve", 1000);
			});
		}
		function showNote(fillId, noteType) {
			...
			$.get("getNote.php", { noteType: noteType, fillId: fillId }, function (result) {
				obj.val(result);
				obj.closest(".divNote").show();
			});
		}
		function showRootCauses(fillId){
			$.get("getRootCauses.php", { fillId: fillId }, function (result) {
				$("#divRootCauses").empty();
				var otherCause = null;
		...
		function showSolutionSteps(fillId){
			$.get("getSolutionSteps.php", { fillId: fillId }, function (result) {
		...
		function showEatMore(fillId){
			$.get("getEatMore.php", { fillId: fillId }, function (result) {
				$("#divEatMore").empty();
				for(var i = 0; i < result.length; i++){
		...
		function fillFoodFilters(fillId){
			$.get("getFoodFilters.php", { fillId: fillId, isTest: (isTest ? 1 : 0) }, function (result) {
		...
		function notify(msg, ms){			
			$("div#msg").text(msg);
			$("div#msg").show();
			setTimeout(function(){ $("div#msg").hide(); }, ms);
		}
            ...
            <td style="vertical-align:top;height:100%;display:flex">
				...
				<div id="divNote2" class="divNote" style="display:none">
					...
					<div id="flexCont2">
						...
						<div class="divInnerNode flexChild1"><span class="spnLabel">MEGOLDANDÓK</span><br><br><div id="divRootCauses"></div></div>
					</div>
					<!-- <div class="divInnerNode"><span class="spnLabel">A MEGOLDÁS LÉPÉSEI</span><br><br><div id="divSolutionSteps"></div></div>
					<div class="divInnerNode"><span class="spnLabel">MIBÕL FOGYASSZ TÖBBET</span><br><br><div id="divEatMore"></div></div> -->
				...
				<div id="divNote22" class="divNote" style="display:none">
					...
					<div class='page2'>
						...
						<div>
							<div class='lowerBoxTitle'>Táplálkozási és életmód ajánlások</div>
					...
					<div class='generateDiv'>
						<input type="button" id="btnGenRecom22" class="generateButton" value='VÉGLEGESÍTÉS ÉS AJÁNLÁS GENERÁLÁS'>
					</div>				
				</div>
				<div id="divNote3" class="divNote" style="display:none">
					...
					<input type="button" id="btnGenFoodRecom2" value="Ú">
					...
					<table class='tblNote3'>
						<tr><th colspan="3">KIZÁRÁSOK</th><th rowspan="2">KIEMELÉSEK</th></tr>
						<tr class="alcim"><th>Élelmiszer csoportok</th><th>Egyéni intoleranciák</th><th>Ízlés</th></tr>
						<tr>
							<td>
								...
								<div class="divInnerNode"><input type="checkbox" id="cbIsMagasHisztamin" value="1"> Hisztamin tartalmú és felszabadító ételek</div>
								...
								<div class="divInnerNode"><input type="checkbox" id="cbIsHusok" value="1"> Húsok, húskészítmények, belsõségek</div>
								<div class="divInnerNode"><input type="checkbox" id="cbIsTojas" value="1"> Tojás, tojáskészítmény</div>
