...
// A TP-HEZ MODIFIKÁLT FÜGGVÉNY - ITT NEM KÉRÜNK HOZZÁ CONSULTANT ID-T!
...
function addOrphanUser($userName, $newConsId){
	...
	$newConsId = (int)$newConsId;
	$userName = str_replace("'", "''", $userName);
	$query = "insert into Questionarie_Fills (userName, CRDTI, ConsultantId, QuestionarieID) values('{$userName}', NOW(), {$newConsId}, 2)";
...
function addNewConsultation($id){
	...
	$query = "insert into Questionarie_Fills (userName, CRDTI, ConsultantId, QuestionarieID, ParentFillId) select userName, now(), ConsultantId, QuestionarieID, $id from Questionarie_Fills where ID = $id";
...
function getUserObj($username,$email)
{
    ...
    //Így kiszûrjük a securitiy kód problémát, ha aposztrófot ír be!
...
//    $subject = "Elkészült egy alfejezet";
    ...
    $body = "Szia! A(z) \"{$subChapter['name']}\" címû gyakorlat új változata elkészült. Ha szívesen megismerkednél vele, a <a href='http://www.healers.digital'>http://www.healers.digital</a> weboldalon 'A Gyakorlatsor' menüpont alatt olvashatod a jelszavaddal való belépés után. Kívánok felismeréseket, bizonyosságod megerõsödését! Lucien";
...
function getClientFinances($userId)
{
    $query = "select * from jelentkezok_finances where user_id = " . (int)$userId . " order by user_date desc";
    $result = mysql_query($query);
    if(!$result){
        print mysql_error();
        exit("Nem sikerült: " . $query);
...
{
    if($id > 0){
        $sql_part = " and id = " . (int)$id;
    }
    $query = "select * from jelentkezok_finances where 1 = 1 {$sql_part} order by user_date desc";
    $result = mysql_query($query);
    if(!$result){
        print mysql_error();
        exit("Nem sikerült: " . $query);
...
function createFinance($userArray, &$message)
{
    ...
    $query = "insert into jelentkezok_finances (user_id, user_date, amount)
                values('{$userArray['user_id']}', '{$userArray['user_date']}', " . (int)$userArray['amount'] . ")";
    ...
    $query = "select max(id) from jelentkezok_finances where user_id = '{$userArray['user_id']}' and user_date = '{$userArray['user_date']}'";
...
function modifyFinance($storeArray, &$message)
{
    ...
    if(isset($storeArray['user_date'])) $fields['user_date'] = "'" . $storeArray['user_date'] . "'";
    if(isset($storeArray['amount'])) $fields['amount'] = (int)$storeArray['amount'];

    $sqlString = array();
    foreach($fields as $key => $value){
        $sqlString[] = "$key = $value";
    }
    $sql = "update jelentkezok_finances set " . implode(', ', $sqlString) . "
                where id = " . (int)$storeArray['id'];
...
{
    $query = "delete from jelentkezok_finances where id = {$financeId}";
    $result = mysql_query($query);
    if(!$result){
        print mysql_error();
        return false;
...
{
    $query = "select sum(amount) from jelentkezok_finances where user_id = " . (int)$userId;
    $result = mysql_query($query);
    if(!$result){
        print $query . ' ' . mysql_error();
        return false;
...
{
    $query = "select l.id_from, b.ID, b.name, b.name_eng, b.name_it, b.done, b.card_number
                from book b
...
{
    $query = "select b2.ID as selectedId, b1.ID, b1.name, b1.name_eng, b1.name_it, b3.ID as sub_ID, b3.name as sub_name, b3.name_eng as sub_name_eng, b3.name_it as sub_name_it, b3.concept, b3.concept_eng, b3.concept_it, 
                    b3.exercise, b3.exercise_eng, b3.exercise_it, b3.done as subDone, b3.crdti
                from book b1
                inner join book b2 on b1.ID = b2.ref_id
...
function getTProfilServerData($id, $type){
    ...
    for($i = 0; $i < count($answers); $i++){
        if($answers[$i]["questionId"] == 87){
            $consultant = utf8_encode($answers[$i]["answer"]);
        }
        if($type == 1 || $answers[$i]["groupId"] == 13){
            $answers[$i]["groupName"] = utf8_encode($answers[$i]["groupName"]);
            $answers[$i]["question"] = utf8_encode($answers[$i]["question"]);
            $answers[$i]["answer"] = utf8_encode($answers[$i]["answer"]);
            $answers[$i]["raw_answer"] = utf8_encode($answers[$i]["raw_answer"]);
            $answers[$i]["questionValues"] = utf8_encode($answers[$i]["questionValues"]);
    ...
    return array(
        'success' => true,
        'userName' => utf8_encode($fill["userName"]),
